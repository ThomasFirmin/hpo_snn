...
class DiehlAndCook2015(AbstractNetwork):
    ...
    def __init__(
        ...
        rrange = torch.arange(self.n_out).reshape(1, self.n_out, 1)
        w = torch.cdist(rrange.float(), rrange.float())
        w /= w.max()
        w *= self.inh
        # Lateral inhibition
        inh_exc_conn = Connection(source=inh_layer, target=exc_layer, w=w.float())
