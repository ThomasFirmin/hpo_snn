...
class NonSpikingPercentageStopping(EarlyStopping):
    ...
    def __call__(self, objective, network):
        ...
        self.count += nsimg
        print(
            f"COUNT: {self.count}/{self.total}>{self.proportion}, PROCESSED: {objective.processed_images}"
        )
...
class SpikePerDatumStopping(SpikeStopping):
    def __init__(self, layer, threshold, patience=1):
        self.threshold = threshold
        condition = (
            lambda objective, network: objective.train_total_spikes_out[self.layer]
            / objective.processed_images
            < threshold
        )
        self._last_zero = 0
    ...
    def __call__(self, objective, network):
        print("AM I STOPPING")
        print(
            objective.train_total_spikes_out[self.layer] / objective.processed_images,
            objective.processed_images,
            objective.train_total_spikes_out[self.layer],
        )
...
class SpikePerDatumLUStopping(SpikeStopping):
    def __init__(self, layer, lower, upper, patience=1):
        self.lower = lower
        self.upper = upper
        condition = lambda objective, network: self._lower_upper(objective, network)
        super(SpikePerDatumLUStopping, self).__init__(layer, condition, patience)

    def _lower_upper(self, objective, network):
        ratio = (
            objective.train_total_spikes_out[self.layer] / objective.processed_images
        )
        return ratio < self.lower and ratio > self.upper
    ...
    def __call__(self, objective, network):
        ...
        return super(SpikePerDatumLUStopping, self).__call__(objective, network)
