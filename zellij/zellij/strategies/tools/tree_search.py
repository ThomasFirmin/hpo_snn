    ...
    def get_next(self):
        ...
        if len(self.open) > 0:
            ...
            for _ in range(idx_min):
                sel_nodes.append(self.open.pop(0))

            self.add_close(sel_nodes)
            return True, sel_nodes
            ...
            for _ in range(idx_min):
                if np.random.random() > self.epsilon:
                    ...
                    if len(self.open) > 1:
                        ...
                        sel_nodes = [].append(self.open.pop(idx))
                    else:
                        sel_nodes = [].append(self.open.pop(0))
...
class Potentially_Optimal_Rectangle(Tree_search):
        ...
        if len(self.open) > 1:
            ...
            if idx:
                for i in reversed(idx):
                    sel_nodes.append(self.open.pop(i))
    ...
    def optimal(self, groups):
        ...
        for key, value in groups:
            ...
            for jdx in range(current_idx + len(subgroup), len(self.open)):
                c = self.open[jdx]
